...
def get_course_subject(course_subject_id: str, db: Session = Depends(get_db)):
...
def update_course_subject(course_subject_id: str, course_subject_update: CourseSubjectUpdate, db: Session = Depends(get_db)):
    course_subject = db.query(CourseSubject).filter(CourseSubject.id == course_subject_id).first()
    if not course_subject:
        raise HTTPException(status_code=404, detail="Course subject not found")
...
def delete_course_subject(course_subject_id: str, db: Session = Depends(get_db)):
    course_subject = db.query(CourseSubject).filter(CourseSubject.id == course_subject_id).first()
    if not course_subject:
        raise HTTPException(status_code=404, detail="Course subject not found")
